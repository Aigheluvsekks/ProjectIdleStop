 ...
 *
 * This captures ONE 0x11F frame and then stops changing.
 *
 * Use this to compare directly against the logic analyzer.
 * -------------------------------------------------------------------------- */
 ...
 * Set:
 *
 *     CAN_RX_CaptureArm = true;
 *
 * from the debugger.
 *
 * The NEXT 0x11F frame will be copied into
 * CAN_RX_CapturedFrame and then frozen.
 * -------------------------------------------------------------------------- */
...
void CAN_Manager_Init(CAN_HandleTypeDef *hcan1)
{
     ...
     *
     * Filter assigns accepted frames to FIFO0.
     * ---------------------------------------------------------------------- */
...
)
{
     ...
     * Drain FIFO0.
     *
     * This is important because several frames can arrive
    ...
    )
    {
            ...
            (incomingMsg.isExtID == false))
        {
             ...
             * FIRST 0x11F CAPTURE
             *
             * This happens ONLY ONCE after reset.
             * --------------------------------------------------------------
             */
             ...
             * MANUAL CAPTURE
             *
             * Set CAN_RX_CaptureArm = true in debugger.
             *
             * The NEXT 0x11F frame is frozen.
             * --------------------------------------------------------------
             */
             ...
             * IMPORTANT
             *
             * Decoder remains disabled while we verify the raw CAN bytes.
             * --------------------------------------------------------------
             */
    ...
    /*
     * If you already have CAN_RX_FIFO0_Level declared in the header,
     * this will show whether frames are accumulating.
     */
